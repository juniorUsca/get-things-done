...
export default function Inbox({ data }) {
  ...
  async function updateTask(id, row) {
    event.preventDefault()
    const res = await fetch(
      `http://localhost:3001/api/inbox/${id}`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...
          <div className="principal">
            <div className="wrap">
              <form onSubmit={newTask}>
                <input type="text" className="tareaInput" placeholder="Agrega tu tarea" name="tarea" />
                            ...
                            editingRow === row
                              ? (
                                <form>
                                  ...
                                  <button type="button" className="btnActualizar" onClick={() => { return updateTask(item._id) }}>Guardar</button>
                                </form>
                              )
                              : (
                                <div>
                                  ...
                                  <button type="button" className="btnEliminar" onClick={() => { return deleteTask(item._id, row) }}>X</button>
                                </div>
                              )
                        }
                    </li>
